...
export class InsightsService {
  ...
  private async getMonthlySpending(orgId: string): Promise<InsightResult> {
    const { data, error } = await this.supabase.rpc('get_month_to_date_total', {
      org_uuid: orgId
    });
  ...
  private async getSpendingTrend(orgId: string): Promise<InsightResult> {
    const { data, error } = await this.supabase.rpc('get_spending_trend', {
      org_uuid: orgId,
      days_back: 30
    });
  ...
  private async getCategoryComparison(orgId: string): Promise<InsightResult> {
    ...
    const [currentData, lastData] = await Promise.all([
      this.supabase.rpc('get_category_breakdown', {
        org_uuid: orgId,
        days_back: currentMonth.getDate()
      }),
      this.supabase
        .from('transactions')
    ...
    const lastMonthTransactions = lastData.data || [];
    
    const lastMonthCategories = lastMonthTransactions.reduce((acc: Record<string, number>, transaction: any) => {
    ...
    const comparison = currentCategories.map((current: any) => {
      const lastAmount = lastMonthCategories[current.category] || 0;
      const change = ((parseFloat(current.amount) - lastAmount) / lastAmount) * 100;
      return {
        category: current.category,
        currentAmount: parseFloat(current.amount),
        lastAmount,
        change: isFinite(change) ? change : 0
      };
    });

    const biggestIncrease = comparison.reduce((max, cat) => 
